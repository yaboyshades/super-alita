...
def cmd_query(args: argparse.Namespace) -> int:
    cfg = load_config()
    base = os.getenv("ALITA_BASE_URL", cfg.get("base_url", "http://127.0.0.1:8080")).rstrip("/")
            ...
            for payload in stream_sse(url, body):
                if isinstance(payload, dict):
                    ...
                    if not printed_header and ev == "start" and args.show_metadata:
                        ...
                        if rl:
                            print(f"[rate-limit] limit={rl.get('limit')} remaining={rl.get('remaining')} reset_in={rl.get('reset_in')}s")
                        printed_header = True
                    # Content frames
...
def main(argv: list[str] | None = None) -> int:
    ...
    q.add_argument("--mode", default="hybrid", choices=["hybrid", "neural", "symbolic"], help="Reasoning mode (advisory)")
    ...
    q.add_argument("--output", choices=["plain", "json", "rich"], default="plain", help="Output format")
    q.add_argument("--show-metadata", action="store_true", help="Include model and session info in output")
    ...
    c.add_argument("--api-header", required=False, default=None, help="Header name for API key (default Authorization)")
    ...
    kc.add_argument("--admin-key", default=None, help="Admin key; defaults to config 'admin_key' or env ALITA_ADMIN_KEY")
    ...
    def cmd_keys_create(args: argparse.Namespace) -> int:
        ...
        headers = _headers()
        admin = args.admin_key or cfg.get("admin_key") or os.getenv("ALITA_ADMIN_KEY", "").strip()
        if admin:
            headers[os.getenv("ALITA_API_HEADER", cfg.get("api_header", "Authorization"))] = f"Bearer {admin}"
        r = requests.post(url, json={"owner": args.owner, "ttl_hours": args.ttl_hours}, headers=headers, timeout=30)
        r.raise_for_status()
        print(r.text)
        return 0
