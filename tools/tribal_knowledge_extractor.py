...
class TribalKnowledgeExtractor:
    def __init__(
        self, project: str = "unknown", output_format: str = "yaml"
    ) -> None:
    ...
    def extract_from_inputs(
        ...
        *,
        ...
        context_lines.append(
            f"Spec: {spec_path or meta.get('spec_file','unknown')}"
        )
        if first.get("question"):
            context_lines.append(
                f"Socratic challenge: {first.get('question')}"
            )
        if first.get("issue"):
        ...
        decision = str(
            first.get("suggested_resolution")
            or "Document resolution details here."
        )
    ...
    def append_to_registry(
        self, registry: dict[str, Any], output_path: Path
    ) -> None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--socratic-report", help="Path to Socratic report (YAML/JSON)"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.socratic_report:
            ste_payload = Path(args.socratic_report).read_text(
                encoding="utf-8"
            )
        ...
        if args.output:
            ...
            if args.format == "json":
                ...
                print(
                    yaml.safe_dump(
                        registry, sort_keys=False, allow_unicode=True
                    )
                )
        return 0
