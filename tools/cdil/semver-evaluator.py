...
def analyze_api_changes(
    spec_diff: dict[str, Any], signature_changes: list[dict[str, Any]]
) -> tuple[str, str]:
    """
    ...
    has_doc_changes = spec_diff.get("description_changes") or spec_diff.get(
        "generator_changes"
    )
    if has_doc_changes:
...
def evaluate_semver_compliance(
    current_version: str, new_version: str, required_bump: str
) -> dict[str, Any]:
    """
...
def main():
    ...
    if len(sys.argv) >= 4:
        ...
        compliance = evaluate_semver_compliance(
            current_version, new_version, required_bump
        )
    ...
    return {"required_bump": required_bump, "reason": reason}
