...
import { registerSDDCommands } from './sdd';
import { PredictiveManager } from './predictiveManager';
import { FeedbackManager } from './feedbackManager';
import { WasmPredictiveAnalyzer } from './predictive/wasmAnalyzer';
import { registerConstitutionalGateway } from './features/constitutionalGateway';
import { ConstitutionalMasteryArchitect } from './constitutionalMasteryArchitect';
import { registerReasoningFeatures } from './reasoning/copilot-context-provider';
import { OptimizedChatProvider } from './ape-engine';
...
const invokeOllama = async (prompt: string): Promise<string> => {
  ...
  return (
    data /* The above code is using optional chaining (`?.`) to access the `content` property of
  the `message` object. If `message` is `null` or `undefined`, it will return an empty
  string `''`. This is a way to safely access nested properties without causing an
  error if any intermediate property is `null` or `undefined`. */?.message?.content || ''
  );
};

const runWasmCalculator = async (a: number, b: number): Promise<number> => {
  try {
    ...
    const addExport = mod.instance.exports.add as
...
export async function activate(ctx: vscode.ExtensionContext) {
  ...
  class DeepCodeDiffProvider implements vscode.TreeDataProvider<DeepCodeFileNode> {
    ...
    getChildren(): DeepCodeFileNode[] {
      ...
      return diffs.map(d => ({
        type: 'file',
        path: String(d.path || d.file || 'unknown'),
        diff: String(d.diff || d.patch || ''),
      }));
    }
    getLatestJson() {
      return this.latest;
  ...
  const dcProvider = new DeepCodeDiffProvider();
  ...
  const checkConnectivity = async (silent = false) => {
    ...
    if (!silent) {
      telemetry?.send('alita/connectivity', { runtime: String(rtOk), llm: String(llmOk) });
    }
    return { rtOk, llmOk };
  };
  ...
  ctx.subscriptions.push(
    vscode.commands.registerCommand('alita.connectivity.ping', async () => {
      const { rtOk, llmOk } = await checkConnectivity(false);
      vscode.window.showInformationMessage(
        `Alita connectivity - Runtime: ${rtOk ? 'OK' : 'FAIL'} | LLM: ${llmOk ? 'OK' : 'FAIL'}`
      );
    })
  );
  ...
  try {
    ...
    if (ext) {
      ...
      if (exists) {
        ...
        telemetry?.send('alita/codegen/metrics', {
          lines: String(lines),
  ...
  // Constitutional Gateway integration
  const constitutionalGateway = registerConstitutionalGateway(ctx);
  disposables.push(constitutionalGateway);

  // Constitutional Mastery Architect v5.0 (agent-mode configuration)
  try {
    const architect = new ConstitutionalMasteryArchitect();
    await architect.initialize(ctx);
  } catch (err) {
    console.warn('Constitutional Mastery Architect failed to initialize:', err);
  }
  ...
  // Commands
  disposables.push(
    vscode.commands.registerCommand('alita.restart', async () => {
      await client.stop();
      await client.start();
    })
  );
  ...
  try {
    (
      ...
      if (
        data &&
        data.__alitaHost &&
        vscode.workspace.getConfiguration('alita').get('codegen.hostTelemetry')
      ) {
    ...
    /* ignore addEventListener absence */
  }
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.invokeAgent', async () => {
      ...
      const progressOpts: vscode.ProgressOptions = {
        ...
        title: 'Invoking Alita Agent (Ollama)...',
      };
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.calc', async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.chatRuntime', async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.codegen.status', async () => {
      try {
        const ext = vscode.extensions.getExtension('super-alita.alita-language-tools');
        if (!ext) {
          return;
        ...
        if (meta.mode === 'generated') {
          try {
            const worldJs = vscode.Uri.joinPath(
              ext.extensionUri,
              'out',
              'src',
              'generated',
              'alita-world.generated.js'
            );
            ...
            const candidate = Object.entries(mod).find(
              ([, val]) =>
                typeof val === 'function' && (val as (...a: unknown[]) => unknown).length <= 1
            );
            if (candidate) {
        ...
        vscode.window.showInformationMessage(
          `WIT bindings: ${meta.mode}. Exports: ${exports}, Lines: ${lines}${smoke}`
        );
      } catch (err) {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.analyze', async () => {
      const cfg = vscode.workspace.getConfiguration('alita');
      const base = (cfg.get<string>('runtime.host') || 'http://127.0.0.1:8080').replace(/\/$/, '');
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.generate', async () => {
      const prompt = await vscode.window.showInputBox({
        prompt: 'DeepCode requirements (e.g., implement feature X)',
      });
      if (!prompt) {
        return;
      }
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.refreshResults', async () => {
      const cfg = vscode.workspace.getConfiguration('alita');
      const base = (cfg.get<string>('runtime.host') || 'http://127.0.0.1:8080').replace(/\/$/, '');
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.showResults', async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.openDiffDoc', async (node?: any) => {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.applySelected', async (node?: any) => {
      ...
      const confirm = await vscode.window.showWarningMessage(
        `Apply proposed changes for ${target.path}?`,
        { modal: true },
        'Apply'
      );
      if (confirm !== 'Apply') {
      ...
      try {
        ...
        vscode.window.showInformationMessage(
          `Apply request sent (files considered: ${result.file_count ?? '?'})`
        );
        telemetry?.send('alita/deepcode/apply', {
          ok: 'true',
          count: String(result.file_count || 0),
        });
      } catch (err) {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.agent.refactorSelection', async () => {
      ...
      await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Alita Refactor' },
        async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('alita.context.indexWorkspace', async () => {
      const folders = vscode.workspace.workspaceFolders;
      if (!folders) {
        return;
      }
      ...
      for (const f of folders) {
        const files = await vscode.workspace.findFiles(
          new vscode.RelativePattern(f, '**/*.{py,ts,rs,alita}'),
          '**/node_modules/**',
          300
        );
      ...
      const endpoint = vscode.workspace
        .getConfiguration('alita')
        .get<string>('context.serverEndpoint');
      const body = {
        files: Object.fromEntries(alitaFiles.map(f => [f.uri.toString(), f.content])),
      };
      try {
        await (globalThis as unknown as { fetch: typeof fetch }).fetch(endpoint + '/index', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  // Register Spec-Driven Development commands
  try {
    disposables.push(...registerSDDCommands(ctx));
  } catch (err) {
    vscode.window.showWarningMessage('Failed to register SDD commands: ' + (err as Error).message);
  }

  // Register DeepCode reasoning features (inline completions + commands)
  try {
    registerReasoningFeatures(ctx);
  } catch (err) {
    vscode.window.showWarningMessage('Failed to register reasoning features: ' + (err as Error).message);
  }

  // Copilot: Open Optimized Chat (APE)
  try {
    const ape = new OptimizedChatProvider();
    disposables.push(
      vscode.commands.registerCommand('alita.copilot.openOptimizedChat', async () => {
        await ape.openOptimizedChat();
      })
    );
  } catch (err) {
    vscode.window.showWarningMessage('Failed to register APE Optimized Chat: ' + (err as Error).message);
  }

  // DeepCode: Toggle reasoning enablement (alias command)
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.toggle', async () => {
      const cfg = vscode.workspace.getConfiguration('alita');
      const enabled = cfg.get<boolean>('reasoning.enabled', true) ?? true;
      await cfg.update('reasoning.enabled', !enabled, vscode.ConfigurationTarget.Workspace);
      vscode.window.showInformationMessage(`DeepCode reasoning ${!enabled ? 'enabled' : 'disabled'}`);
    })
  );

  // DeepCode: Open configuration (alias command)
  disposables.push(
    vscode.commands.registerCommand('alita.deepcode.configure', async () => {
      await vscode.commands.executeCommand('workbench.action.openSettings', 'alita.reasoning');
    })
  );
