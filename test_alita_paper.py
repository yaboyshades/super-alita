    ...
    if result.get('status') == 'complete' and result.get('applied'):
        ...
        for applied_item in result.get('applied', []):
            if isinstance(applied_item, dict) and 'paths' in applied_item:
                ...
                main_files = [p for p in applied_item['paths'] 
                ...
                if main_files:
                    try:
                        ...
                        if main_file.exists():
                            ...
                            components = [
                                'MultiModalFusion', 'ConversationalMemory', 
                                'InformationSeeking', 'AdversarialTraining',
                                'HierarchicalAttention', 'NeuralSymbolic'
                            ]
                            found_components = [c for c in components 
                                              if c in content or c.lower().replace('_', '') in content.lower()]
                            if found_components:
