# Continuous monitoring script for Super Alita development
# Monitors health, logs, and provides interactive commands
...
Write-Host "📊 Super Alita Development Monitor" -ForegroundColor Cyan
...
function Test-ServerHealth {
    try {
        ...
        if ($response.status -eq "healthy") {
            ...
            Write-Host "[$timestamp] ⚠️  Server unhealthy: $($response.status)" -ForegroundColor Yellow
            return $false
        }
    }
    catch {
        $timestamp = Get-Date -Format "HH:mm:ss"
        Write-Host "[$timestamp] ❌ Server unreachable" -ForegroundColor Red
        return $false
    }
}
...
function Show-QuickCommands {
    Write-Host ""
    Write-Host "🛠️  Quick Commands (use GitHub Copilot CLI for safe suggestions):" -ForegroundColor Blue
    Write-Host "  ghcs 'check server logs'                    - Get log checking commands" -ForegroundColor Cyan
    Write-Host "  ghcs 'test Super Alita endpoints'          - Get testing commands" -ForegroundColor Cyan
    Write-Host "  ghcs 'restart development server'          - Get restart commands" -ForegroundColor Cyan
    Write-Host "  ghce 'curl http://127.0.0.1:8080/healthz'  - Explain health check" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "💡 Direct commands:" -ForegroundColor Blue
    Write-Host "  curl http://127.0.0.1:8080/tools/catalog   - List available tools" -ForegroundColor White
    Write-Host "  python validate_deployment.py              - Run full validation" -ForegroundColor White
    Write-Host ""
}

# Initial health check
Write-Host "Performing initial health check..." -ForegroundColor Blue
$isHealthy = Test-ServerHealth

if (!$isHealthy) {
    Write-Host ""
    Write-Host "⚠️  Server appears to be down. Consider starting it first:" -ForegroundColor Yellow
    Write-Host "  .\scripts\start-dev.ps1" -ForegroundColor White
    Write-Host "  OR use: ghcs 'start Super Alita development server'" -ForegroundColor Cyan
}
    ...
    while ($true) {
        ...
        if ($isHealthy) {
            ...
            if ($consecutiveFailures -eq 3) {
                ...
                Write-Host "💡 Suggested action: ghcs 'diagnose Super Alita server issues'" -ForegroundColor Magenta
                Write-Host ""
            }
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA3FqCMt19+vsBP
# T8tBlhUSEZ/knQwxdZEvYm2j3GZeD6CCFRswggHdMIIBRqADAgECAhB1LVe8LkJa
...
# MQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgwKQMOeaWO7pYKOFTWgxO
# 5eXpQToZKEH+EtSC38cBRsEwDQYJKoZIhvcNAQEBBQAEgYAZ1rp7r6ZfSHNoWhGN
# IPIUULmiV3gjFNmOwwB5h2sX+q/TR3eoII1R0c9Zf69p2ylqJFZEErjhi3CsLf8T
# oLgYUGVKG0ngiGfiLgTCMl3SuPkwrjdgZc3CpsfqdmfMjdVoAwYriLbUxSUWPmOF
# DSCQiPALvjjCqjjTPtClZOUoSqGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MjgwNTI3MzZaMC8GCSqGSIb3DQEJBDEiBCAu+Yo/NNu5Bf4fz+c8qnh0sFOt41kj
# mBfybVSQxazkFjANBgkqhkiG9w0BAQEFAASCAgCC4O5zoS4NPtXfJiuad290lvqq
# 2ML1g10KcVVCumNYvk038fo7Lqof8nh62txkEhXHJYJ70Ogb8TDBK+fIzstqG2ml
# neeW5EOTrryfDQK5PIL5iCDIFquHpV8pyH/JpqeE1jYw0W0qel2VV5kwHUQfyP9u
# 8GKEtloxx3C7LsOq39la/P0hGs0OPr1ylqbwqHHI4v1HCjVjUk5sU7QscVj3NwDV
# Ka7SNKHvQHPBQMOGGrlC2PYgf+Bgttivz9PuyBkRKtfsDiUZAA9cSev42AIxx8jd
# 44r6rNDfCDab5I4yJ7rWibDxi048wW53NoSFDc5VoOWWyZ4D7yxZL2pwYAOcy3DA
# PLABiXWwKaJrbTMmQI94WZbHz0a4TGmtmlzk/JHqrj48vC7BngNkfqypQqhYGxxn
# JcTC8PUmhztq56ct9zWqpobLrPWPnFqcXzjvE8FSKYwW+tRlHbJEt3hPe6zkKM/B
# khm3KpBMspOAysFnpfBLOGwdtYsMNaqoAWuRFOjLSxqo0JpOea7rQ0qdi3qF/GYj
# +eZIHs42/Qd1Segvp1ZzJ5GZhiPfo/kJxZUJ4dn8AAOnspSyLPr9I0mlnFeaNNU8
# BJ1epD3bjh+O4sRwRjjw69k7h3Dfryy7pnOE2KplchpiZNj4WRO06H2VWqLSskc+
# Ilym+GkEaVbvFec7NA==
