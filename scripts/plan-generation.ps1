function New-TechnicalPlan {
    param(
        [string]$TechStackPrompt,
        [string]$FeaturePath,
        [string]$SpecificationPath
    )
    $Specification = Get-Content $SpecificationPath -Raw
    $Constitution = Get-Content "memory/constitution.md" -Raw
    $PlanTemplate = Get-Content "templates/plan-template.md" -Raw
    $PlanningContext = @"
...
Generate a comprehensive technical plan that:
1. Respects all constitutional constraints
2. Implements all specification requirements
3. Uses the specified technology stack
4. Includes security, performance, and scalability considerations
5. Provides detailed implementation steps
"@
    $TechnicalPlan = $PlanTemplate -replace '\[.*?\]', $TechStackPrompt
    $PlanPath = Join-Path $FeaturePath "plan.md"
    Set-Content -Path $PlanPath -Value $TechnicalPlan
    git add $FeaturePath
    git commit -m "feat: add technical implementation plan"
    Write-Host "✅ Technical plan created with supporting artifacts"
}

# SIG # Begin signature block
# MIIaSQYJKoZIhvcNAQcCoIIaOjCCGjYCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAUZKmPpKAZT5ro
# 9nQ0qcUOZ2Wex/DSaG2tzsWy6oE2DqCCFRswggHdMIIBRqADAgECAhB1LVe8LkJa
...
# MQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgb/Kx5uphPN7W5w7+8Lsk
# sie7WKJprhUTJ/UWMWC0PkQwDQYJKoZIhvcNAQEBBQAEgYAyuPeNZsN68v63retf
# 7UuwxoIgEwRnxuaLHgVTZEsIzGk0Pw4luoe61Fa6qqVPJtsX3EbY/QbdwGHYiEU3
# 8m5m32B/zY7CiLnyXo6xpk/a2yRIjQuL7mKJZB6O6HozCmLqukQcpmlMVcadPHjo
# Nuiqjaz1FnZCkJCJZ1OZkW8mdKGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MjEwNzMzMDFaMC8GCSqGSIb3DQEJBDEiBCDoNWlrf9sXhWcAEQ2tdkvmbgScOggj
# WdHJlEdcFjWQgjANBgkqhkiG9w0BAQEFAASCAgAqoAFpk7OU+4NtAXS8U8WfaaE+
# E2Jxg/uleYFiTWhGHdOGtfWP2X6DgJPbXf9D33ySLBKV25rhgmYGjJmpYAUYl7bQ
# 6pRcGJy0SIOjjDNFCkGOLgVoPCDMd6nQWCJWLrv0CPSOhfzHL4LMPFJVsKuPBmYg
# r1XKngoiC0+5IdO1Gn0PMd8IccB6AdULrrpg9lCaZsVkFlmk619cEA6DHwQZfgCW
# qm+HFGkUQjH9yLqlZHrH9ltxtjjKRf6RACsh8pnAS40NX+MKJnjpMgurZD1VE9A0
# IAhqrwdwYQMDf2xsYUlxNXhzwoZKS5RkVtZKLrzsaz4t3SyQJW+9D14mTf69t7w7
# f/PDhlvaMih1rAAovjYUVsoJcMON4Aggd6Sadkb+ItO9PhoQZ1fq9u9PsP5e/ctq
# 4GCed/izKfe8GHYPqOs7j2BAF8Zsj8oUFA5O6MkFe6kXLp4n/17AE3vbFkdo4StU
# kVc7+KFODN4rYJo4hqQAAUtEINfsICsfWUNiXxsPtWXpO2YgeF6cCwh4iYvnQzhV
# QLKwr8rHG7LwqeoQ/o3FnZ8fK9kAHnAx+kYBfrlhHZ0xrguTEJjl4CsbNweJKjnY
# PwKOHorKCOcFWWb+wVPve1eWw7iBhKgta8K0JN4taD1on1S5Gkdz311liY97wjSZ
# /L8GhNrkqQAmyxllCg==
