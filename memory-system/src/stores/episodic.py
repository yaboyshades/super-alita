...
class _SimpleIndex:
    """Fallback vector index mimicking a small subset of FAISS API."""
    ...
    def search(self, queries: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
        if not self._vectors:
            empty = np.zeros((len(queries), k), dtype=np.float32)
            return empty, -np.ones_like(empty, dtype=np.int64)

        vector_stack = np.vstack(self._vectors)
        scores = []
        indices = []
        for query in queries:
            sims = vector_stack @ query / (
                np.linalg.norm(vector_stack, axis=1) * np.linalg.norm(query) + 1e-8
            )
            order = np.argsort(-sims)[:k]
            pad = np.pad(order, (0, max(0, k - len(order))), constant_values=-1)
            pad_scores = np.pad(
                sims[order], (0, max(0, k - len(order))), constant_values=0.0
            )
            scores.append(pad_scores)
            indices.append(pad)
...
class EpisodicStore:
    ...
    def __init__(self, persist_path: str = "storage/episodic"):
        ...
        if faiss is not None:
            ...
            self.index = _SimpleIndex(self.dimension)
