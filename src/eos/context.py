...
class CynefinClassifier:
    ...
    def extract_features(self,
        ...
        clarity_indicators = ["because", "therefore", "results in", "causes",
                              "leads to", "deterministic", "predictable"]
        chaos_indicators = ["uncertain", "unknown", "unpredictable",
                            "volatile", "complex", "emergent", "adaptive"]
        ...
        predictable_indicators = ["standard", "established", "proven",
                                  "routine"]
        unpredictable_indicators = ["novel", "innovative", "experimental",
                                    "unprecedented"]
        
        pred_score = sum(1 for indicator in predictable_indicators
                         if indicator in problem_text)
        unpred_score = sum(1 for indicator in unpredictable_indicators
                           if indicator in problem_text)
        ...
        est_count = sum(1 for hint in domain_hints 
                       for domain in established_domains 
                       if domain in hint.lower())
        emerg_count = sum(1 for hint in domain_hints 
        ...
        time_indicators = ["urgent", "immediate", "asap", "deadline", "quickly"]
        time_pressure_score = sum(1 for constraint in constraints 
        ...
        if len(stakeholders) > 0:
            ...
            features.stakeholder_agreement = max(0.1, 1.0 - (len(stakeholders) - 1) * 0.2)
        
        # Knowledge gaps from uncertainty
        uncertainty_indicators = ["unclear", "unknown", "investigate", "research"]
        gap_score = sum(1 for constraint in constraints + [problem_text]
        ...
        if risk_scores:
            ...
            risk_avg = sum(1 if r == "high" else 0.5 if r == "medium" else 0 
                          for r in risk_scores) / len(risk_scores)
    ...
    def classify(self, problem_context: Dict[str, Any],
        ...
        if prior_distribution:
            for domain in domain_scores:
                ...
                domain_scores[domain] = (0.7 * domain_scores[domain] + 
                                       0.3 * prior_weight)
        
        # Convert scores to probabilities using softmax
        max_score = max(domain_scores.values())
        exp_scores = {domain: math.exp(score - max_score) 
                     for domain, score in domain_scores.items()}
        total_exp = sum(exp_scores.values())
        
        probabilities = {domain: exp_score / total_exp 
                        for domain, exp_score in exp_scores.items()}
        
        # Find primary domain
        primary_domain = max(probabilities.keys(), 
                           key=lambda d: probabilities[d])
...
class ContextAnalyzer:
    ...
    def analyze_context(self, eos_spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        uncertainty_thresholds = context_config.get("uncertainty_thresholds", {})
        entropy_threshold = uncertainty_thresholds.get("entropy_promote_lift", 0.9)
        chaotic_threshold = uncertainty_thresholds.get("chaotic_emergency", 0.4)
        
        should_lift = classification.entropy > entropy_threshold
        chaotic_mode = classification.probabilities.get(
            CynefinDomain.CHAOTIC, 0.0
        ) > chaotic_threshold
        
        return {
            "cynefin_classification": {
                "probabilities": {domain.value: prob 
    ...
    def _recommend_methods(self, classification: CynefinClassification) -> List[str]:
        ...
        for domain, prob in probabilities.items():
            if domain != primary and prob > 0.25:  # Significant secondary domain
                if domain == CynefinDomain.COMPLICATED and "DMAIC" not in methods:
                    methods.append("DMAIC")
                elif domain == CynefinDomain.COMPLEX and "SYSTEMS" not in methods:
    ...
    def update_context(self, current_context: Dict[str, Any], 
        ...
        if evidence_domain in updated_probs:
            ...
            updated_probs[evidence_domain] = min(1.0, 
                updated_probs[evidence_domain] + evidence_strength
            )
        ...
        updated_context["cynefin_classification"]["probabilities"] = updated_probs
        
        # Recalculate primary domain and entropy
        primary_domain = max(updated_probs.keys(), key=lambda k: updated_probs[k])
        entropy = -sum(p * math.log2(p) if p > 0 else 0 
                      for p in updated_probs.values())
        
        updated_context["cynefin_classification"]["primary_domain"] = primary_domain
