...
class EOSMangleOrchestrator:
        ...
        try:
            ...
            if self.mangle_ability and self.config.enable_deductive_validation:
                constitutional_score = (
                    await self._validate_constitutional_compliance(result)
                )
                result.governance_report["mangle_constitutional_score"] = (
                    constitutional_score
                )
        ...
        try:
            ...
            for rule in constitutional_rules:
                await self.mangle_ability.add_rule(
                    f"constitutional_rule_{len(constitutional_rules)}", rule
                )
    ...
    async def _execute_enhanced_orchestration(self) -> OrchestrationResult:
        ...
        if self.mangle_ability and self.config.enable_mangle_routing:
            self.eos_orchestrator.moe_router = MangleEnhancedRouter(
                self.eos_orchestrator.moe_router, self.mangle_ability
            )
    ...
    async def _validate_constitutional_compliance(
        ...
        try:
            ...
            if (
                hasattr(compliance_result, "success")
                and compliance_result.success
            ):
...
class MangleEnhancedRouter:
    ...
    async def execute_expert(
        self, expert_id: str, inputs: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
...
async def run_eos_mangle_orchestration(
    spec_file_path: str, config: EOSMangleConfig = None
) -> OrchestrationResult:
...
def main():
    ...
    try:
        result = asyncio.run(
            run_eos_mangle_orchestration(args.spec_file, config)
        )
        ...
        result_dict = {
            ...
            "mangle_constitutional_score": result.governance_report.get(
                "mangle_constitutional_score", 0
            ),
