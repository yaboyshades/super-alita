...
class EOSValidator:
    ...
    def validate_yaml(self, yaml_path: Path) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                valid=False, errors=[f"YAML parsing error: {e}"]
            )
        except FileNotFoundError:
            return ValidationResult(
                valid=False, errors=[f"File not found: {yaml_path}"]
            )
    ...
    def _validate_semantics(
        self, spec_data: dict[str, Any], result: ValidationResult
    ) -> None:
...
class EOSSchema:
    ...
    def load_and_validate(
        self, yaml_path: Path
    ) -> tuple[ValidationResult, EOSSpec | None]:
