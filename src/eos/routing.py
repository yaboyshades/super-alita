...
class ExpertGating:
    ...
    def __init__(self, routing_config: dict[str, Any]):
        ...
        self.weights = self.scoring_config.get(
            "weights",
            {"fit": 0.4, "expected_utility": 0.3, "cost": 0.2, "risk": 0.1},
        )
    ...
    def _calculate_fit_score(
        self, expert: dict[str, Any], current_state: str, current_method: str
    ) -> float:
    ...
    def _calculate_utility_score(
        self, expert_id: str, expert: dict[str, Any], context: dict[str, Any]
    ) -> float:
    ...
    def _calculate_cost_score(
        self, expert: dict[str, Any], context: dict[str, Any]
    ) -> float:
    ...
    def _calculate_ucb1_bonus(self, expert_id: str) -> float:
        ...
        n_total = sum(
            h.get("usage_count", 0) for h in self.expert_usage_history.values()
        )
    ...
    def apply_gating_rules(
        self, expert_selections: list[ExpertSelection], context: dict[str, Any]
        ...
        for selection in expert_selections:
            ...
            for rule in self.gating_rules:
                ...
                if self._evaluate_gating_condition(
    ...
    def _evaluate_gating_condition(
        self,
        condition: str,
    ...
    def update_expert_history(
        ...
        history["avg_quality"] = (
            current_avg * (count - 1) + quality_score
        ) / count
...
class MoERouter:
    ...
    def __init__(
        self, experts: list[dict[str, Any]], routing_config: dict[str, Any]
    ):
    ...
    async def route_to_experts(
        ...
        for expert_id, expert in self.experts.items():
            ...
            if not self._check_input_compatibility(
                required_inputs, expert_inputs
            ):
                reasoning.append(
        ...
        # Apply gating rules
        filtered_selections = self.gating.apply_gating_rules(
            selections, context
        )
        ...
        if backup_experts:
            ...
            backup_scores = [
                expert.score.total_score for expert in backup_experts
            ]
    ...
    def _check_input_compatibility(
        self, required: list[str], provided: list[str]
    ) -> bool:
    ...
    async def _execute_tool(
        ...
        return {
            ...
            "quality_score": 0.8,
            "metadata": {"expert_type": "tool", "expert_id": expert["id"]},
        }
    ...
    async def _execute_agent(
        ...
        return {
            ...
            "quality_score": 0.85,
            "metadata": {"expert_type": "agent", "expert_id": expert["id"]},
        }
    ...
    async def _execute_human(
        ...
        return {
            ...
            "quality_score": 0.9,
            "metadata": {"expert_type": "human", "expert_id": expert["id"]},
        }
