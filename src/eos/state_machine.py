...
class Guard(ABC):
    ...
    @abstractmethod
    def evaluate(self, context: StateContext, spec: dict[str, Any]) -> bool:
...
class ContextShiftGuard(Guard):
    ...
    def evaluate(self, context: StateContext, spec: dict[str, Any]) -> bool:
        ...
        return (
            context.chaotic_mode
            and context.metrics.get("p_chaotic", 0.0) > tau_emergency
        )
...
class SufficiencyGuard(Guard):
    ...
    def evaluate(self, context: StateContext, spec: dict[str, Any]) -> bool:
        ...
        return (
            context.evidence_score >= tau_evidence
            and context.contradiction_debt <= tau_contra
        )
...
class BudgetGuard(Guard):
    ...
    def evaluate(self, context: StateContext, spec: dict[str, Any]) -> bool:
        ...
        for _resource, remaining in context.budget_remaining.items():
...
class State(ABC):
    ...
    def can_enter(self, context: StateContext) -> bool:
        ...
        return all(
            guard.evaluate(context, self.spec) for guard in self.entry_guards
        )

    def can_exit(self, context: StateContext) -> bool:
        """Check if state can be exited"""
        return all(
            guard.evaluate(context, self.spec) for guard in self.exit_guards
        )
...
class AnalyzeState(State):
    ...
    def __init__(self, spec: dict[str, Any]):
        ...
        self.entry_guards = [
            LambdaGuard(lambda ctx, spec: not ctx.chaotic_mode)
        ]
...
class Transition:
    ...
    def can_transition(
        self, context: StateContext, spec: dict[str, Any]
    ) -> bool:
...
class EOSStateMachine:
    ...
    def _create_transitions(self) -> list[Transition]:
        ...
        transitions.append(
            Transition(
                ...
                condition=lambda ctx, spec: bool(
                    ctx.metrics.get("new_signals_discovered", False)
                ),
            )
        )
        ...
        transitions.append(
            Transition(
                ...
                transition_type=TransitionType.CYCLE,
                condition=lambda ctx, spec: ctx.metrics.get("success_kpis", 0)
                < spec.get("evaluation", {}).get("target_kpis", 0.8),
            )
        )
                ...
                transitions.append(
                    Transition(
                        ...
                        condition=lambda ctx, spec: (
                            ctx.chaotic_mode
                            and ctx.metrics.get("p_chaotic", 0)
                            > spec.get("context", {})
                            .get("uncertainty_thresholds", {})
                            .get("chaotic_emergency", 0.4)
                        ),
    ...
    def _decide_transition(self) -> TransitionDecision | None:
        ...
        for transition in self.transitions:
            if (
                transition.from_state == current_type
                and transition.transition_type == TransitionType.EMERGENCY
                and transition.can_transition(self.context, self.spec)
            ):
    ...
    def _should_stop(self) -> bool:
        ...
        for condition in stopping_conditions:
            ...
            if condition_str == "all_targets_met":
                ...
                if (
                    self.context.budget_remaining.get("compute_gh", 1.0) <= 0
                    or self.context.metrics.get("marginal_gain", 1.0) < 0.02
                ):
                    return True
