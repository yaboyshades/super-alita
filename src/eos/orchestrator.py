...
class EOSOrchestrator:
    ...
    async def _execute_adaptive_orchestration(
        self, initial_state_context: StateContext
    ) -> StateContext:
        ...
        while iteration < max_iterations:
            ...
            self._trace_event(
                "state_transition",
                {
                    ...
                    "decision": (
                        decision.target_state.value if decision else None
                    ),
                    "iteration": iteration,
                },
            )
    ...
    async def _determine_required_operators(
        self, state_type: StateType, context: StateContext
    ) -> list[OperatorType]:
        ...
        if state_type == StateType.OBSERVE:
            ...
            if self.current_context and self.current_context.get(
                "recommendations", {}
    ...
    async def _execute_operator_with_routing(
        self, operator_type: OperatorType, state_context: StateContext
    ) -> Any:
        ...
        try:
            ...
            if operator_type in [
                OperatorType.SYNTHESIZE,
                OperatorType.DECOMPOSE,
            ]:
    ...
    def _determine_current_method(self) -> str:
        ...
        if self.current_context:
            methods = self.current_context.get("recommendations", {}).get(
                "methods", []
            )
    ...
    async def _merge_operator_results(
        self, state_context: StateContext, operator_result: Any
    ) -> StateContext:
    ...
    async def _convert_expert_to_operator_result(
        self, expert_result: dict[str, Any], operator_context: OperatorContext
    ) -> Any:
        ...
        return OperatorResult(
            ...
            reasoning=[
                f"Expert execution: {expert_result.get('outputs', {}).get('result', '')}"
            ],
    ...
    async def _should_stop_orchestration(self, context: StateContext) -> bool:
        ...
        for metric in evaluation_config.get("metrics", []):
            ...
            if metric_id == "time_to_insight":
                ...
                actual_minutes = context.metrics.get(
                    "total_duration_ms", 0
                ) / (1000 * 60)
    ...
    async def _generate_results(
        self, final_context: StateContext, start_time: float
    ) -> OrchestrationResult:
    ...
    def get_orchestration_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "current_state": (
                self.state_machine.current_state.state_type.value
                if self.state_machine.current_state
                else None
            ),
...
def main():
    ...
    try:
        ...
        result_dict = {
            ...
            "governance_score": result.governance_report.get(
                "compliance_score", 0
            ),
