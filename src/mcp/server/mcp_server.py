...
async def query_knowledge_graph(
    ...
    match query_type:
        ...
        case _:
            raise ValueError(
                "query_type must be semantic, structural, or temporal"
            )
...
async def get_system_telemetry(
    metric_type: str, time_range: str = "1h"
) -> dict[str, Any]:
...
def _gather_performance_metrics() -> dict[str, Any]:
    ...
    python_files = (
        sum(1 for _ in src_root.rglob("*.py")) if src_root.exists() else 0
    )
    test_modules = (
        sum(1 for _ in tests_root.rglob("test_*.py"))
