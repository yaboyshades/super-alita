...
class GitHubMCPTools:
    ...
    def __init__(
        ...
        *,
        ...
        if self.github_token:
            ...
            logger.warning(
                "GitHub token not configured – API calls will be rate limited or disabled."
            )
    ...
    async def create_issue(
        ...
        *,
        ...
        return await self._request(
            "POST", f"/repos/{repo}/issues", json=payload
        )
    ...
    async def create_pull_request(
        ...
        *,
        ...
        return await self._request(
            "POST", f"/repos/{repo}/pulls", json=payload
        )
    ...
    async def analyze_super_alita_context(
        ...
        if analysis_type == "architecture":
            ...
            summary["dependencies"] = await self._load_repo_files(
                repo_root, files
            )
        elif analysis_type == "patterns":
            summary["patterns"] = self._describe_code_patterns()
        elif analysis_type == "config":
            env_template = await self._load_single_file(
    ...
    async def _load_repo_files(
        self, root: Path, files: Iterable[str]
    ) -> dict[str, str | None]:
    ...
    async def _load_single_file(self, root: Path, relative: str) -> str | None:
        ...
        try:
            data = await self._request(
                "GET", f"/repos/yaboyshades/super-alita/contents/{relative}"
            )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request(
                ...
                if response.status >= 400:
                    logger.error(
                        "GitHub API error %s: %s", response.status, text
                    )
                try:
...
def register_github_tools(
    app: Any, *, tools: GitHubMCPTools | None = None
) -> GitHubMCPTools:
    ...
    if getattr(app, "_github_tools_registered", False):
        ...
        return (
            existing
            if isinstance(existing, GitHubMCPTools)
            else GitHubMCPTools()
        )
    ...
    async def github_create_issue(
        ...
        return await _wrap(github_tools.create_issue)(
            repo=repo, title=title, body=body or "", labels=labels or []
        )
