...
router: APIRouter = APIRouter(prefix="/tools/execute/super_alita")
...
async def get_agent_telemetry_handler(
    ...
    if event_type:
        filtered_events = [
            e for e in _events if e.get("event_type") == event_type
        ]
...
async def send_telemetry_handler(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not all(
        k in params for k in ["event_type", "source", "data", "timestamp"]
    ):
        raise HTTPException(
...
async def mem0_get_all_memories_handler(
    ...
    if category:
        filtered_memories = [
            m for m in _memories if m.get("category") == category
        ]
...
async def mem0_search_memories_handler(
    params: dict[str, Any],
) -> dict[str, Any]:
...
async def mem0_store_session_learning_handler(
    params: dict[str, Any],
) -> dict[str, Any]:
