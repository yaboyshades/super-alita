...
class SecurityResilienceMiddleware:
    ...
    def __init__(
        ...
        self.policy_manager = (
            policy_manager or create_policy_manager_from_env()
        )
        self.secret_manager = secret_manager or get_global_secret_manager()
        self.resilience_manager = (
            resilience_manager or get_global_resilience_manager()
        )
        ...
        **kwargs,
        ...
        if not can_call:
            ...
            raise PermissionError(
                f"Access denied: {actor} not authorized to call {tool_name}"
            )
        ...
        for key, value in args.items():
            if isinstance(value, str) and value.startswith("secret:"):
                ...
                if secret_value is None:
                    logger.warning(
                        f"Secret '{secret_name}' not found, using fallback"
                    )
    ...
    def _determine_service_category(self, tool_name: str) -> ServiceCategory:
        ...
        if any(
            ...
            keyword in tool_lower
            for keyword in ["openai", "llm", "gpt", "anthropic"]
        ):
            return ServiceCategory.LLM_API
        elif any(
            keyword in tool_lower
            for keyword in ["db", "database", "redis", "sql"]
        ):
            return ServiceCategory.DATABASE
        elif any(
            keyword in tool_lower
            for keyword in ["file", "fs", "read", "write"]
        ):
            return ServiceCategory.FILE_SYSTEM
        elif any(
            keyword in tool_lower for keyword in ["process", "cpu", "compute"]
        ):
    ...
    async def get_health_status(self) -> dict[str, Any]:
        ...
        return {
            "policy_manager": {"status": "healthy", "metrics": policy_metrics},
            "secret_manager": {
                "status": (
                    "healthy" if any(secret_health.values()) else "degraded"
                ),
            ...
            "resilience_manager": {
                "status": (
                    "healthy"
                    if resilience_health["health_score"] > 0.5
                    else "degraded"
                ),
...
class SecureAbilityRegistry:
    ...
    def __init__(
        self,
        base_registry,
        middleware: SecurityResilienceMiddleware | None = None,
    ):
    ...
    async def execute(
        ...
        if hasattr(self.base_registry, "execute"):
            ...
            def original_func():
                return self.base_registry.execute(tool_name, args)
...
def integrate_security_resilience(app):
    ...
    if hasattr(app.state, "ability_registry"):
        ...
        app.state.ability_registry = SecureAbilityRegistry(
            original_registry, middleware
        )
        logger.info(
            "Ability registry wrapped with security & resilience protection"
        )
    ...
    async def reload_policies():
        ...
        return {
            "success": success,
            "message": (
                "Policies reloaded" if success else "Failed to reload policies"
            ),
        }
...
async def test_integration():
    ...
    test_cases = [
        ...
        (
            "llm_client",
            "openai_completion",
            {"prompt": "test", "api_key": "secret:openai_key"},
        ),
    ]
