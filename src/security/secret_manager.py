...
class EnvironmentSecretBackend(SecretBackendInterface):
    ...
    def _load_env_file(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.env_vars)} environment variables from {self.env_file}"
            )
...
class AWSSSMSecretBackend(SecretBackendInterface):
    ...
    def __init__(
        self, region: str | None = None, prefix: str = "/super-alita/"
    ):
    ...
    async def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                raise RuntimeError(
                    "boto3 library required for AWS backend. Install with: pip install boto3"
                )
    ...
    async def get_secret(self, name: str) -> str | None:
        ...
        try:
            ...
            logger.error(
                f"AWS SSM parameter retrieval failed for '{name}': {e}"
            )
            return None
...
class SecretManager:
    ...
    def __init__(
        ...
        if enable_encryption:
            ...
            if not self.encryption_key:
                ...
                logger.warning(
                    "Generated ephemeral encryption key for secret cache"
                )
    ...
    def _detect_backends(self) -> list[SecretBackendInterface]:
        ...
        if os.getenv("VAULT_ADDR") or os.getenv("VAULT_TOKEN"):
            ...
            backends.append(
                VaultSecretBackend(vault_url, vault_token, mount_point)
            )
    ...
    async def get_secret(self, name: str) -> str | None:
        ...
        for backend in self.backends:
            try:
                ...
                logger.error(
                    f"Backend {type(backend).__name__} failed for secret '{name}': {e}"
                )
                await self._audit_log(
                    "backend_error",
                    name,
    ...
    async def list_secrets(self) -> dict[str, list[str]]:
        ...
        for backend in self.backends:
            try:
                ...
                logger.error(
                    f"Backend {type(backend).__name__} listing failed: {e}"
                )
    ...
    async def health_check(self) -> dict[str, bool]:
        ...
        for backend in self.backends:
            try:
                ...
                logger.error(
                    f"Health check failed for {type(backend).__name__}: {e}"
                )
    ...
    async def _cache_secret(
        self, name: str, value: str, backend: SecretBackendInterface
    ):
        ...
        if len(self.cache) > 1000:
            ...
            sorted_items = sorted(
                self.cache.items(), key=lambda x: x[1].cached_at
            )
