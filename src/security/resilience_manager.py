...
class CircuitBreakerManager:
    ...
    def create_circuit_breaker(
        ...
        def fallback_fn(*args, **kwargs):
            ...
            logger.warning(f"Circuit breaker {service_name} is open, executing fallback")
            context = {"service": service_name, "args": args, "kwargs": kwargs}
            ...
            if fallback_strategy:
                ...
                if hasattr(fallback_strategy, 'default_response'):
                    ...
                    return {"error": f"Service {service_name} is temporarily unavailable"}
            
            return {"error": f"Service {service_name} is temporarily unavailable"}
        
        def notify_fn(breaker, transition):
            """Notification function for state changes"""
            logger.info(f"Circuit breaker {service_name} transitioned to {transition}")
            if service_name in self.service_metrics:
                if transition == "open":
                    self.service_metrics[service_name].circuit_breaker_trips += 1
        
        circuit_breaker = pybreaker.CircuitBreaker(
            fail_max=config.failure_threshold,
            reset_timeout=config.recovery_timeout,
            exclude=config.expected_exception if config.expected_exception != Exception else None,
    ...
    async def call_with_circuit_breaker(
        ...
        **kwargs
        ...
        if not circuit_breaker:
            ...
            return await self._execute_and_measure(service_name, func, *args, **kwargs)
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                try:
                    result = circuit_breaker(lambda: asyncio.run(async_wrapper()))()
...
class BulkheadManager:
    ...
    def create_bulkhead(
        ...
        logger.info(f"Created bulkhead for {category.value} with {config.max_concurrent} workers")
    ...
    async def execute_in_bulkhead(
        ...
        **kwargs
        ...
        if category not in self.semaphores:
            ...
            logger.warning(f"No bulkhead configured for {category.value}, executing directly")
            if asyncio.iscoroutinefunction(func):
                return await func(*args, **kwargs)
            else:
        ...
        try:
            ...
            if semaphore._waiters and len(semaphore._waiters) >= config.max_queue_size:
                ...
                raise Exception(f"Bulkhead {category.value} queue is full, rejecting request")
    ...
    def get_bulkhead_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for category, config in self.configs.items():
            ...
            status[category.value] = {
                ...
                "waiting_count": len(semaphore._waiters) if semaphore._waiters else 0,
...
class RequestHedger:
    ...
    async def execute_with_hedging(
        ...
        **kwargs
        ...
        primary_task = asyncio.create_task(self._execute_safely(primary_func, *args, **kwargs))
...
class ResilienceManager:
    ...
    def _infer_category(self, service_name: str) -> ServiceCategory:
        ...
        if any(keyword in name_lower for keyword in ["llm", "openai", "anthropic", "gpt"]):
            return ServiceCategory.LLM_API
        elif any(keyword in name_lower for keyword in ["github", "api", "http", "rest"]):
            return ServiceCategory.EXTERNAL_API
        elif any(keyword in name_lower for keyword in ["db", "database", "sql", "redis"]):
            return ServiceCategory.DATABASE
        elif any(keyword in name_lower for keyword in ["file", "fs", "disk", "io"]):
...
if __name__ == "__main__":
    ...
    async def test_resilience_manager():
        ...
        for i in range(total_calls):
            try:
                result = await rm.execute_resilient_call(
                    "test_service",
                    unreliable_service,
                    ServiceCategory.EXTERNAL_API,
                    use_hedging=True,
        ...
        for service, status in health["circuit_breakers"].items():
            ...
            print(f"  {state_emoji} {service}: {status['state']} (success: {status['success_rate']:.2f})")
