...
class GitHubApiClient:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if request.method.upper() == "GET":
                    response = await client.get(url, headers=headers, params=request.parameters)
                    ...
                    response = await client.delete(url, headers=headers, params=request.parameters)
                else:
    ...
    def _update_rate_limit_info(self, response: httpx.Response):
        ...
        try:
            ...
            if reset_timestamp:
                self.rate_limit_reset = datetime.fromtimestamp(int(reset_timestamp), timezone.utc)
    ...
    async def extract_priority_metrics(
        ...
        try:
            if item_type == "pull_request":
                # Get PR details
                pr_response = await self.make_request(GitHubApiRequest(
                ...
                if pr_response.success and pr_response.data:
                    ...
                    stakeholder_keywords = ["@team", "@admin", "@maintainer", "urgent", "critical"]
                    metrics.has_stakeholder_mention = any(
                        keyword in body.lower() for keyword in stakeholder_keywords
                    )
                    
                    # Get CI status
                    status_response = await self.make_request(GitHubApiRequest(
                ...
                files_response = await self.get_pull_request_files(repository, item_number)
                if files_response.success and files_response.data:
                    # Check if this PR blocks others by modifying critical files
                    critical_files = ["package.json", "requirements.txt", "Dockerfile", "Makefile"]
                ...
                # Get issue details
                issue_response = await self.make_request(GitHubApiRequest(
