...
class ConstitutionalValidator:
    ...
    def __init__(
        self, compliance_level: ComplianceLevel = ComplianceLevel.STANDARD
    ):
        self.compliance_level = compliance_level
        self.violation_patterns = self._initialize_violation_patterns()
        self.assessment_cache = {}

    def _initialize_violation_patterns(
        self,
    ...
    async def assess_constitutional_compliance(
        ...
        with constitutional_processing_time.labels(
            ...
            if context:
                contextual_violations = (
                    await self._check_contextual_violations(content, context)
                )
    ...
    async def _check_violation_patterns(
        self, content: str, patterns: list[dict[str, Any]], violation_type: str
            ...
            try:
                ...
                if matches:
                    violation = ConstitutionalViolation(
                        ...
                        evidence=[
                            str(match) for match in matches[:3]
                        ],  # Limit evidence
    ...
    async def _check_contextual_violations(
        ...
        if target_audience == "children" or "child" in target_audience.lower():
            if any(
                ...
                for term in ["violence", "adult", "inappropriate"]
            ):
                violations.append(
        ...
        if content_type == "educational":
            if any(
                term in content.lower()
                for term in ["biased", "unsubstantiated", "false"]
            ):
                violations.append(
                    ConstitutionalViolation(
    ...
    def _calculate_compliance_score(
        ...
        normalized_penalty = min(total_penalty, max_possible_penalty) / max(
            max_possible_penalty, 1.0
        )
    ...
    def _determine_compliance(
        ...
        if (
            critical_violations
            and self.compliance_level != ComplianceLevel.PERMISSIVE
        ):
            return False
...
class ConstitutionalDeepConfPipeline(EnhancedDeepConfPipeline):
    ...
    async def process_constitutional_consensus_request(
        ...
        try:
            ...
            constitutional_assessment = await self.constitutional_validator.assess_constitutional_compliance(
                base_result.get("consensus_text", ""), context=context
            )
            ...
            if filtered_responses:
                ...
                for response in filtered_responses:
                    response_assessment = await self.constitutional_validator.assess_constitutional_compliance(
                        response.get("text", ""), context=context
                    )
                    filtered_scores.append(
                        response_assessment.compliance_score
                    )
            ...
            self.compliance_stats["total_violations"] += len(
                constitutional_assessment.violations
            )
            ...
            result.metadata.update(
                {
                    "constitutional_compliance": {
                        ...
                        "compliance_level": self.compliance_level.value,
                        "filtered_count": (
                            len(individual_responses) - len(filtered_responses)
                            if individual_responses
                            else 0
                        ),
                    },
    ...
    async def batch_constitutional_assessment(
        ...
        for text in texts:
            task = (
                self.constitutional_validator.assess_constitutional_compliance(
                    text, context
                )
            )
            tasks.append(task)
        ...
        valid_assessments = [
            a for a in assessments if isinstance(a, ConstitutionalAssessment)
        ]
    ...
    def get_compliance_stats(self) -> dict[str, Any]:
        ...
        return {
            **self.compliance_stats,
            "compliance_rate": self.compliance_stats["compliant_responses"]
            / total_assessments,
            "filtering_rate": self.compliance_stats["filtered_responses"]
            / total_assessments,
            "avg_violations_per_assessment": self.compliance_stats[
                "total_violations"
            ]
            / total_assessments,
            "compliance_level": self.compliance_level.value,
...
def create_constitutional_deepconf_pipeline(
    ...
    **kwargs,
    ...
    return ConstitutionalDeepConfPipeline(
        model_api=model_api, compliance_level=compliance_level, **kwargs
    )
