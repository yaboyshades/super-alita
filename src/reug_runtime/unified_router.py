...
class UnifiedRouter:
    ...
    async def execute_turn(self, 
        ...
        try:
            ...
            async with asyncio.timeout(self.executor.timeout_config.overall_timeout_s):
                async for event in self.base_router.execute_turn(message, session_id, **kwargs):
                ...
                "tools": {
                    tool_name: {
                        ...
                        "last_failure_age_s": (
                            ...
                            if circuit.last_failure_time > 0 else None
                        )
                    }
...
async def create_resilient_router(base_router: Any, 
    ...
    """
    router = UnifiedRouter(base_router, retry_config, timeout_config, circuit_config)
    try:
        yield router
    finally:
        await router.close()
...
def load_resilience_config_from_env() -> tuple[RetryConfig, TimeoutConfig, CircuitBreakerConfig]:
    ...
    timeout_config = TimeoutConfig(
        ...
        connection_timeout_s=float(os.getenv("REUG_CONNECTION_TIMEOUT_S", "10.0"))
    )
    
    circuit_config = CircuitBreakerConfig(
        failure_threshold=int(os.getenv("REUG_CIRCUIT_FAILURE_THRESHOLD", "5")),
        recovery_timeout_s=int(os.getenv("REUG_CIRCUIT_RECOVERY_TIMEOUT_S", "60")),
        half_open_max_calls=int(os.getenv("REUG_CIRCUIT_HALF_OPEN_CALLS", "3"))
    )
