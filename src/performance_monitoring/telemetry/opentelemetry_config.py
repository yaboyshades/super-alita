...
class OpenTelemetryCollector:
    ...
    def start_span(self, 
        ...
        if not trace_id:
            trace_id = (
                _telemetry_context.get_current_trace_id() or str(uuid4())
            )
    ...
    def _check_slo_violations(self, span: TelemetrySpan) -> None:
        ...
        if (span.duration_ms is not None and
                span.duration_ms > self.slos.latency_p95_ms):
            violations.append({
        ...
        return {
            ...
            "slo_compliance": {
                "latency_p95_compliant": (
                    ...
                    if p95_idx < len(latencies) else True
                ),
