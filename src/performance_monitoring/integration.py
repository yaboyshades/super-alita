...
class PerformanceMonitoringSystem:
    ...
    def __init__(
        ...
        self.performance_monitor = PerformanceMonitor(
            metrics_retention_hours=self.config.get(
                "metrics_retention_hours", 24
            ),
            collection_interval_seconds=self.config.get(
        ...
        self.telemetry_bridge = TelemetryBridge(
            ...
            flush_interval_seconds=self.config.get(
    ...
    async def validate_commit(
        self, commit_message: str, changed_files: list[str], diff_data: str
    ) -> dict[str, Any]:
        ...
        logger.info(
            f"Validating commit with {len(changed_files)} changed files"
        )
    ...
    async def validate_code_change(
        ...
        try:
            compliance_score = (
                await self.constitutional_engine.validate_code_change(
    ...
    def get_constitutional_dashboard(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_validations": self.quality_pipeline.get_execution_history(
                5
            ),
        }
    ...
    async def run_health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            health_status["component_health"]["performance_monitor"] = {
                ...
                "interactions_count": perf_summary.get(
    ...
    def _setup_quality_gates(self) -> None:
        ...
        constitutional_gate = ConstitutionalGate(
            self.constitutional_engine, threshold=self.constitutional_threshold
        )
        ...
        performance_thresholds = self.config.get(
            ...
            {"response_time_ms": 1000, "success_rate": 0.95},
        )
        performance_gate = PerformanceGate(
            self.performance_monitor, performance_thresholds
        )
        self.quality_pipeline.add_gate(performance_gate)

        # Security gate
        security_config = self.config.get(
            "security_config", {"check_dependencies": True, "check_code": True}
        )
    ...
    async def _integration_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                self.dashboard.update_performance_data(
                    self.performance_monitor
                )
                self.dashboard.update_constitutional_data(
        ...
        while self._running:
            try:
                ...
                if health_status["overall_health"] != "healthy":
                    logger.warning(
                        f"System health: {health_status['overall_health']}"
                    )
...
async def create_monitoring_system(
    config_path: Path | None = None, constitutional_threshold: float = 0.75
    ...
    system = PerformanceMonitoringSystem(
        config=config, constitutional_threshold=constitutional_threshold
    )
