...
class ComplianceScore:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "article_scores": {
                k.value: v for k, v in self.article_scores.items()
            },
            "violations": [
                {
...
class ConstitutionalEngine:
    ...
    def __init__(self, compliance_threshold: float = 0.75):
        ...
        logger.info(
            f"Constitutional Engine initialized with threshold {compliance_threshold}"
        )
    ...
    async def validate_compliance(
        ...
        logger.info(
            f"Constitutional validation complete: {overall_score:.3f} "
    ...
    def get_compliance_trend(self, hours: int = 24) -> dict[str, Any]:
        ...
        recent_scores = [
            score
            for score in self.compliance_history
        ...
        return {
            ...
            "compliance_rate": sum(1 for s in recent_scores if s.is_compliant)
            / len(recent_scores),
    ...
    async def _validate_article(
        ...
        if not violations:
            ...
            deductions = sum(
                self._get_severity_deduction(v.severity) for v in violations
            )
    ...
    def _calculate_overall_score(
        self, article_scores: dict[ConstitutionalArticle, float]
    ) -> float:
    ...
    def _get_severity_deduction(self, severity: str) -> float:
        ...
        deductions = {"critical": 0.5, "high": 0.3, "medium": 0.2, "low": 0.1}
    ...
    def _calculate_trend(self, scores: list[float]) -> str:
        ...
        older_avg = (
            sum(scores[:-5]) / max(1, len(scores) - 5)
            if len(scores) > 5
            else scores[0]
        )
    ...
    def _initialize_validation_rules(
        self,
    ) -> dict[ConstitutionalArticle, list]:
    ...
    async def _validate_library_usage(
        ...
        if data.get("type") == "code_change":
            ...
            for change in changes:
                if "class " in change and (
                    "http" in change.lower() or "json" in change.lower()
                ):
    ...
    async def _validate_dependency_management(
        ...
        if data.get("file_path", "").endswith(
    ...
    async def _validate_interface_simplicity(
        self, data: dict[str, Any], context: dict[str, Any]
    ) -> ComplianceViolation | None:
