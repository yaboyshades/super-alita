    ...
    async def validate(self, context: dict[str, Any]) -> QualityGateResult:
        ...
        try:
            ...
            if context.get("type") == "commit":
                compliance_score = (
                    await self.constitutional_engine.validate_commit(
                ...
                compliance_score = (
                    await self.constitutional_engine.validate_compliance(
                        context
                    )
                )
    ...
    async def validate(self, context: dict[str, Any]) -> QualityGateResult:
        ...
        try:
            ...
            if avg_response_time > response_threshold:
                violations.append(
                    f"Response time {avg_response_time:.2f}ms exceeds threshold {response_threshold}ms"
                )
            ...
            if success_rate < success_threshold:
                violations.append(
                    f"Success rate {success_rate:.3f} below threshold {success_threshold}"
                )

            # Calculate overall performance score
            response_score = min(
                1.0, response_threshold / max(avg_response_time, 1)
            )
...
class SecurityGate:
    ...
    async def validate(self, context: dict[str, Any]) -> QualityGateResult:
        ...
        try:
            ...
            critical_violations = sum(
                1 for v in violations if "critical" in v.lower()
            )
            score = max(
                0.0,
                1.0 - (critical_violations * 0.5) - (len(violations) * 0.1),
            )
        ...
        try:
            ...
            if result.returncode != 0 and result.stdout:
                ...
                for vuln in safety_data:
                    violations.append(
                        f"Security vulnerability in {vuln.get('package', 'unknown')}: {vuln.get('advisory', 'No details')}"
                    )
    ...
    async def _check_python_security(self, file_path: str) -> list[str]:
        ...
        try:
            ...
            if "exec(" in content:
                violations.append(
                    f"Potential code injection in {file_path}: exec() usage"
                )
            if "eval(" in content:
                violations.append(
                    f"Potential code injection in {file_path}: eval() usage"
                )
            if "os.system(" in content:
                violations.append(
                    f"Potential command injection in {file_path}: os.system() usage"
                )
...
class QualityGatePipeline:
    ...
    async def execute_pipeline(
        ...
        logger.info(
            f"Executing quality gate pipeline with {len(self.gates)} gates"
        )
        ...
        gate_results = await asyncio.gather(
            *gate_tasks, return_exceptions=True
        )

        for i, result in enumerate(gate_results):
            if isinstance(result, Exception):
                logger.error(
                    f"Gate {self.gates[i].name} failed with exception: {result}"
                )
        ...
        if overall_passed:
            logger.info(
                f"Quality gate pipeline PASSED in {execution_time:.2f}ms"
            )
        else:
            logger.warning(
                f"Quality gate pipeline FAILED in {execution_time:.2f}ms"
            )
            for result in results:
                if not result.passed:
                    logger.warning(
                        f"  {result.gate_name}: {len(result.violations)} violations"
                    )
    ...
    def get_gate_statistics(self) -> dict[str, Any]:
        ...
        for execution in self.execution_history:
            for gate_result in execution["gate_results"]:
                ...
                stats["total_execution_time"] += gate_result[
                    "execution_time_ms"
                ]
        ...
        for gate_name, stats in gate_stats.items():
            stats["success_rate"] = (
                stats["passed_executions"] / stats["total_executions"]
            )
            stats["average_execution_time"] = (
                stats["total_execution_time"] / stats["total_executions"]
            )
            stats["average_violations"] = (
                stats["total_violations"] / stats["total_executions"]
            )
