    ...
    def should_allow_request(self) -> bool:
        ...
        if self.state == CircuitBreakerState.CLOSED:
            ...
            if (self.last_failure_time and 
                (now - self.last_failure_time).total_seconds() > self.timeout_seconds):
...
class ServiceRegistry:
    ...
    async def register_service(self, service_name: str, instance: ServiceInstance):
        ...
        async with self._lock:
            if instance not in self.services[service_name]:
                ...
                logger.info(f"Registered service instance: {service_name} -> {instance.endpoint}")
        ...
        async with self._lock:
            ...
            for i, instance in enumerate(instances):
                if instance.id == instance_id:
                    ...
                    logger.info(f"Deregistered service instance: {service_name} -> {removed.endpoint}")
    ...
    async def update_instance_status(self, service_name: str, instance_id: str, 
            ...
            for instance in self.services[service_name]:
                if instance.id == instance_id:
                    ...
                    if old_status != status:
                        logger.info(f"Service {service_name}/{instance_id} status: {old_status} -> {status}")
                    break
...
class AutoScaler:
    ...
    async def _evaluate_scaling_rule(self, service_name: str, rule: ScalingRule):
        ...
        if last_action:
            ...
            if (avg_value > rule.threshold_scale_up and 
                time_since_action < rule.scale_up_cooldown):
                return
            
            if (avg_value < rule.threshold_scale_down and 
                time_since_action < rule.scale_down_cooldown):
                return
    ...
    async def _scale_service(self, service_name: str, rule: ScalingRule, 
                           direction: str, metric_value: float):
        """Execute scaling action"""
        try:
            if hasattr(self, '_scale_callback'):
                await self._scale_callback(service_name, rule, direction, metric_value)
            
            # Record action time
            action_key = f"{service_name}:{rule.metric_name}"
            self.last_scale_action[action_key] = datetime.now()
            
            logger.info(f"Scaled {service_name} {direction} due to {rule.metric_name}={metric_value}")
...
class ScalabilityManager:
    ...
    async def get_service_instance(self, service_name: str, 
        ...
        instances = await self.service_registry.get_service_instances(service_name, healthy_only=True)
        ...
        return await self.load_balancer.select_instance(service_name, instances, context)
    
    async def record_request_result(self, service_name: str, instance_id: str, 
                                  success: bool, response_time_ms: float):
        """Record request result for monitoring"""
        # Update instance stats
        instances = await self.service_registry.get_service_instances(service_name, healthy_only=False)
        ...
        self.auto_scaler.record_metric(service_name, 'response_time', response_time_ms)
        self.auto_scaler.record_metric(service_name, 'success_rate', 1.0 if success else 0.0)
    
    def add_scaling_rule(self, service_name: str, metric_name: str,
    ...
    async def _handle_scaling_event(self, service_name: str, rule: ScalingRule,
                                  direction: str, metric_value: float):
        ...
        if direction == 'up' and current_count < rule.max_instances:
            logger.info(f"Would scale up {service_name} (metric: {rule.metric_name}={metric_value})")
            # In real implementation, would trigger instance creation
            
        elif direction == 'down' and current_count > rule.min_instances:
            logger.info(f"Would scale down {service_name} (metric: {rule.metric_name}={metric_value})")
    ...
    async def _check_instance_health(self, service_name: str, instance: ServiceInstance):
        ...
        try:
            ...
            is_healthy = instance.success_rate > 0.8 and instance.response_time_ms < 1000
            ...
            await self.service_registry.update_instance_status(service_name, instance.id, status)
