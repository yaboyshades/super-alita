...
class ResourceMonitor:
    ...
    def register_resource_pool(
        ...
        logger.info(
            f"Registered resource pool: {name} ({resource_type}, max: {max_size})"
        )
    ...
    def get_resource_summary(self) -> dict[str, Any]:
        ...
        for name, pool in self.resource_pools.items():
            pool_stats[name] = {
                ...
                "in_use": pool.in_use,
                "utilization": (
                    pool.in_use / pool.max_size if pool.max_size > 0 else 0
                ),
...
class GarbageCollectionOptimizer:
    ...
    def force_collection(self) -> dict[str, int]:
        ...
        logger.info(
            f"GC collection freed {stats['objects_freed']} objects in {stats['collection_time_ms']:.2f}ms"
        )
    ...
    def get_gc_stats(self) -> dict[str, Any]:
        ...
        avg_time = sum(s["collection_time_ms"] for s in recent_stats) / len(
            recent_stats
        )
