        ...
        with self._lock:
            ...
            return {
                **self._stats,
                ...
                'hit_rate': hit_rate,
                'avg_size_bytes': self._stats['size_bytes'] / len(self._cache) if self._cache else 0
            }
...
class PerformanceOptimizer:
    ...
    def register_connection_pool(self, name: str, connection_factory: Callable, 
        ...
        self.connection_pools[name] = ConnectionPool(connection_factory, config)
        ...
        for op_name, timings in self._performance_profiles.items():
            if timings:
                stats['performance_profiles'][op_name] = {
                    ...
                    'recent_avg_ms': sum(timings[-10:]) * 1000 / min(len(timings), 10)
                }
        ...
        while self._running:
            try:
                ...
                for pool_name, pool_stats in stats['connection_pools'].items():
                    ...
                    if utilization > 0.8:
                        logger.warning(f"High connection pool utilization ({pool_name}): {utilization:.2%}")
