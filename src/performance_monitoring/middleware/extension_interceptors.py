...
class ExtensionTelemetryMiddleware:
    ...
    def track_extension_call(
        self, extension_id: str, interaction_type: str = "query"
    ) -> Callable[[F], F]:
...
def track_extension_call(
    extension_id: str, interaction_type: str = "query"
) -> Callable[[F], F]:
    """Decorator to automatically track extension calls."""
    return _extension_middleware.track_extension_call(
        extension_id, interaction_type
    )
...
@asynccontextmanager
async def extension_interaction_context(
    extension_id: str, interaction_type: str = "query"
):
...
class PerformanceThresholdMonitor:
    ...
    def check_slo_violations(self) -> list[dict[str, Any]]:
        ...
        if self.telemetry_collector:
            ...
            # Check latency SLO
            if (
                summary.get("latency_ms", {}).get("p95", 0)
