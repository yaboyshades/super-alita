...
class VSCodeBridgeSimulator:
    ...
    def __init__(self, orchestrator_url: str = "http://localhost:8080", auth_token: str = "dev-token"):
            ...
            try:
                ...
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        print("[VSCode Bridge] Received successful response from orchestrator.")
                        self._simulate_ide_actions(result)
                    else:
                        error_text = await response.text()
                        print(f"[VSCode Bridge] Error from orchestrator ({response.status}): {error_text}")
            except aiohttp.ClientConnectorError:
                print("[VSCode Bridge] Error: Could not connect to the orchestrator service.")
                print("                 Please ensure the FastAPI service is running.")
    ...
    def _simulate_ide_actions(self, orchestrator_response: Dict[str, Any]):
        ...
        if snippets:
            ...
            for i, snippet in enumerate(snippets):
                print(f"   - Snippet {i+1} (prefix: '{snippet.get('prefix')}'):")
                print(f"     Body: {snippet.get('body', '').replace(chr(10), ' ')}")
