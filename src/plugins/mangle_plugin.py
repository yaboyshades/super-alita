...
class ManglePlugin(PluginInterface):
    ...
    def __init__(self):
        ...
        # Mangle binary path - prefer MANGLE_BIN, fallback to MANGLE_BIN_PATH, then PATH
        self.mangle_bin = os.environ.get("MANGLE_BIN") or os.environ.get("MANGLE_BIN_PATH") or "mangle"
    ...
    async def execute_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                stdout = await proc.arun([self.mangle_bin, "run", program_path], timeout=30)
                return self._parse_mangle_output(stdout)
            finally:
                os.unlink(program_path)
        except proc.ProcError as e:
            logger.error(f"Mangle execution error: {e.stderr}")
            raise MangleException(f"Execution error: {e.stderr}")
        except Exception as e:
