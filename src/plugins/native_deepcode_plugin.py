...
class NativeDeepCodePlugin(PluginInterface):
    ...
    async def start(self) -> None:
        ...
        await self.subscribe(
            "autogen_capability_needed", self._handle_capability_request
        )
    ...
    async def _native_deepcode_request(
        ...
        result = await self._generate_native_implementation(
            task_kind, requirements, repo_path
        )
        ...
        try:
            self._results_cache.write_text(
                json.dumps(self._latest_results, indent=2), encoding="utf-8"
            )
    ...
    async def _generate_native_implementation(
        ...
        if task_kind == "web_scraper":
            ...
            return await self._generate_generic_capability(
                task_kind, requirements, repo_path
            )

    async def _generate_web_scraper(
...
'''
    ...
    async def _generate_paper2code(
        self, requirements: str, repo_path: str
    ) -> dict[str, Any]:
...
'''
    ...
    def _generate_adaptive_tests(
        self, base_name: str, concepts: dict[str, list[str]]
    ) -> str:
...
'''
    ...
    async def _generate_text2backend(
        self, requirements: str, repo_path: str
    ) -> dict[str, Any]:
        """Generate generic backend implementation"""
        return await self._generate_generic_capability(
            "backend", requirements, repo_path
        )

    async def _generate_generic_capability(
        self, task_kind: str, requirements: str, repo_path: str
    ) -> dict[str, Any]:
        ...
        if self._results_cache.exists():
            try:
                cached_data = json.loads(
                    self._results_cache.read_text(encoding="utf-8")
                )
