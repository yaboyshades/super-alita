...
class PerplexicaSearchPlugin(PluginInterface):
    ...
    async def setup(
        ...
        cfg_path = (
            Path(__file__).resolve().parents[1] / "config" / "perplexica.yaml"
        )
        try:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        self.web_agent_health_url, timeout=5
                    ) as resp:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "PerplexicaSearchPlugin started - ready for AI-powered search"
        )
    ...
    async def _initialize_llm_client(self) -> None:
        ...
        try:
            ...
            if api_key and api_key != "your-key-here":
                ...
                logger.warning(
                    "No valid GEMINI_API_KEY found - reasoning will be simplified"
                )
    ...
    async def _handle_search_request(self, event) -> None:
        ...
        try:
            ...
            logger.info(
                f"🔍 Processing Perplexica search: '{query}' (mode: {search_mode})"
            )
            ...
            tool_result = ToolResultEvent(
                ...
                tool_call_id=data.get(
                    "tool_call_id", f"perplexica_{session_id}"
                ),
                session_id=session_id,
                success=True,
            ...
            logger.info(
                f"✅ Perplexica search completed in {response.processing_time:.2f}s"
            )
            ...
            error_result = ToolResultEvent(
                ...
                tool_call_id=data.get(
                    "tool_call_id",
                    f"perplexica_error_{data.get('session_id', 'default')}",
                ),
                session_id=data.get("session_id", "default"),
                success=False,
                result={"error": str(e)},
    ...
    async def _fallback_search(
        ...
        try:
            ...
            if self.web_agent and hasattr(self.web_agent, "_searxng_search"):
                results = await self.web_agent._searxng_search(
                    query, max_results
                )
...
"""
            ...
            if not reasoning:
                reasoning = (
                    f"AI analysis of {len(results)} results for '{query}'"
                )
            if not citations:
                citations = [
    ...
    def _prepare_llm_context(
        ...
        for i, result in enumerate(
            results[:5]
        ):  # Limit to top 5 for context window
