...
class EnhancedPriorityMetrics(BaseModel):
    ...
    urgency: float = Field(default=5.0, ge=1.0, le=10.0, description="Task urgency score") 
    effort: float = Field(default=5.0, ge=1.0, le=10.0, description="Required effort estimate")
    ...
    deadline_hours: float | None = Field(default=None, description="Hours until deadline")
    
    # Social factors
    stakeholder_count: int = Field(default=0, ge=0, description="Number of stakeholders")
    community_interest: float = Field(default=1.0, ge=0.0, description="Community interest level")
    
    # Technical factors
    complexity_score: float = Field(default=5.0, ge=1.0, le=10.0, description="Technical complexity")
    risk_score: float = Field(default=5.0, ge=1.0, le=10.0, description="Risk assessment")
...
class GitHubPriorityCalculator:
    ...
    def _calculate_github_adjustments(self, metrics: EnhancedPriorityMetrics) -> float:
        ...
        if any(label.lower() in critical_labels for label in github_metrics.issue_labels):
    ...
    def _calculate_temporal_adjustments(self, metrics: EnhancedPriorityMetrics) -> float:
        ...
        if metrics.age_hours > 0:
            age_factor = 1 + (metrics.age_hours * self.temporal_weights["age_penalty"] / 24)
        ...
        if metrics.deadline_hours is not None:
            if metrics.deadline_hours <= 0:
                ...
                adjustment *= self.temporal_weights["deadline_urgency"]  # Within 24 hours
    ...
    def get_priority_explanation(
        ...
        if metrics.stakeholder_count > 3:
            factors.append(f"High stakeholder involvement ({metrics.stakeholder_count})")
