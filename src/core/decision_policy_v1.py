...
class DecisionPolicyEngine:
    ...
    def register_capability(self, capability: CapabilityNode) -> None:
    ...
    async def decide_and_plan(
        self, message: str, ctx: dict[str, Any], budget: Budget | None = None
    ) -> ExecutionPlan:
    ...
    def safe_fallback_plan(self, goal: Goal, budget: Budget | None) -> ExecutionPlan:
        ...
        plan_steps = [
            ...
            {
                "say": (
                    "However, I don't have sufficient capabilities to "
                    "complete this safely."
                )
            },
            {
                "say": (
                    "Could you provide more specific instructions or "
...
class GoalSynthesizer:
    ...
    def synthesize(
        self, intent: IntentType, slots: dict[str, Any], _ctx: dict[str, Any]
    ) -> Goal:
...
class UtilityCalculator:
    ...
    def calculate(
        self, capability: CapabilityNode, goal: Goal, _ctx: dict[str, Any]
    ) -> float:
        ...
        # Utility formula: p_success*Reward - α*latency - β*cost - γ*risk + explore
        utility = (
            p_success * reward
...
class PlanBuilder:
    ...
    def build_guardrail_plan(self, _goal: Goal) -> list[dict[str, Any]]:
...
def create_example_plan_for_bootstrap() -> dict[str, Any]:
    ...
    return {
        ...
        "plan": [
            ...
            {
                "tool": "git.clone_or_pull",
                "args": {
                    "repo_url": "https://github.com/yaboyshades/super-alita.git",
                    "path": "~/super-alita",
                },
                "assign": "repo",
            },
            {
                "tool": "env.ensure_dotenv",
                "args": {"cwd": "~/super-alita", "template": ".env.example"},
                "assign": "env",
            },
            {
                "tool": "deps.install",
            ...
            {
                "tool": "http.get",
                "args": {"url": "http://localhost:8080/healthz", "timeout": 5},
                "assign": "health",
            },
            {
                "if": {"gte": ["{{health.status}}", 200, 299]},
                "say": "Service healthy (200).",
            },
            {
                "else": [
