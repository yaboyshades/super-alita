...
class SuperAlitaAgentServicer(pb2_grpc.SuperAlitaAgentServicer):
    ...
    def GetStatus(
        ...
        try:
            ...
            active_plugins = 0
            if self.cortex_runtime:
                active_plugins += 1
            if self.telemetry_collector:
                active_plugins += 1
            if self.knowledge_plugin:
                active_plugins += 1
            if self.optimization_plugin:
                active_plugins += 1
        ...
        try:
            if not self.cortex_runtime:
                ...
                return pb2.TaskResponse(
                    ...
                    success=False,
                    error_message="Cortex unavailable",
                )
            ...
            # Process the task
            result = await self.cortex_runtime.process_cycle(
                request.content, cortex_context
            )
    ...
    def GetMetrics(
        ...
        try:
            ...
            return pb2.MetricsResponse(
                metric_families=pb_families, collected_at=collected_timestamp
            )
    ...
    async def CreateConcept(
        self, request: pb2.CreateConceptRequest, context: grpc.ServicerContext
    ) -> pb2.CreateConceptResponse:
        ...
        try:
            if not self.knowledge_plugin:
                ...
                return pb2.CreateConceptResponse(
                    success=False, error_message="Knowledge graph unavailable"
                )
    ...
    async def CreateRelationship(
        ...
        try:
            ...
            return pb2.CreateRelationshipResponse(
                relationship_id=relationship_id, success=True
            )
    ...
    async def CreatePolicy(
        self, request: pb2.CreatePolicyRequest, context: grpc.ServicerContext
    ) -> pb2.CreatePolicyResponse:
        ...
        try:
            if not self.optimization_plugin:
                ...
                return pb2.CreatePolicyResponse(
                    success=False, error_message="Optimization unavailable"
                )
    ...
    async def MakeDecision(
        ...
        try:
            ...
            return pb2.DecisionResponse(success=False, error_message=str(e))

    async def ProvideFeedback(
        self, request: pb2.FeedbackRequest, context: grpc.ServicerContext
    ) -> pb2.FeedbackResponse:
    ...
    def GetOptimizationStats(
        ...
        try:
            ...
            for policy_id, policy_data in (
                stats.get("policies", {}).get("policies", {}).items()
            ):
                if policy_data and "bandit" in policy_data:
                    arm_stats = []
                    for arm_id, arm_data in (
                        policy_data["bandit"].get("arms", {}).items()
                    ):
...
class SuperAlitaGrpcServer:
    ...
    def __init__(
        self, host: str = "localhost", port: int = 50051, max_workers: int = 10
    ):
        self.host = host
        self.port = port
