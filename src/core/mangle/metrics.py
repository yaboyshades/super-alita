...
class PrometheusMetricsCollector:
    ...
    def set_component_health(self, component: str, healthy: bool) -> None:
        ...
        self._ensure_initialized()
        self._metrics["component_health"].labels(component=component).set(
            1 if healthy else 0
        )
    ...
    def observe_cortex_cycle_duration(
        self, session_id: str, module: str, duration: float
    ) -> None:
        """Record Cortex cycle duration."""
        self._ensure_initialized()
        self._metrics["cortex_cycle_duration"].labels(
    ...
    def inc_events_emitted(self, event_type: str, source_plugin: str) -> None:
        ...
        self._ensure_initialized()
        self._metrics["events_emitted_total"].labels(
            event_type=event_type, source_plugin=source_plugin
        ).inc()
    ...
    def inc_knowledge_operations(
        ...
        self._ensure_initialized()
        self._metrics["knowledge_operations_total"].labels(
            operation_type=operation_type, status=status
        ).inc()

    def observe_knowledge_operation_duration(
        self, operation_type: str, duration: float
    ) -> None:
        """Record knowledge operation duration."""
        self._ensure_initialized()
        self._metrics["knowledge_operation_duration"].labels(
    ...
    def inc_optimization_decisions(
        self, policy_id: str, algorithm: str, arm_id: str
    ) -> None:
    ...
    def inc_optimization_rewards(self, policy_id: str, arm_id: str) -> None:
        ...
        self._ensure_initialized()
        self._metrics["optimization_rewards_total"].labels(
            policy_id=policy_id, arm_id=arm_id
        ).inc()

    def observe_optimization_reward_value(
        self, policy_id: str, arm_id: str, reward: float
    ) -> None:
        """Record optimization reward value."""
        self._ensure_initialized()
        self._metrics["optimization_reward_value"].labels(
            policy_id=policy_id, arm_id=arm_id
        ).observe(reward)

    def set_optimization_arm_performance(
        self, policy_id: str, arm_id: str, metric: str, value: float
    ) -> None:
    ...
    def observe_plugin_execution_duration(
        self, plugin_name: str, method: str, duration: float
    ) -> None:
