"""Optimized mutation-score enforcement middleware for Copilot Agent mode.
...
OPTIMIZATIONS FOR COMPLEX FILES:
- File complexity detection and adaptive strategies
- Targeted test selection based on file paths
- Configurable timeouts to prevent hanging
- Early termination for consistently high scores
...
_PYTEST_TIMEOUT = int(os.getenv("MUTANT_GATE_TIMEOUT", "60"))
# lines threshold for complex files
_COMPLEX_FILE_THRESHOLD = int(os.getenv("MUTANT_GATE_COMPLEX_LINES", "300"))
# mutants to check before early termination
_EARLY_TERMINATION_SAMPLES = int(os.getenv("MUTANT_GATE_EARLY_SAMPLES", "5"))
...
def analyse_file(path: Path, *, max_mutants: int = _MAX_MUTANTS) -> MutationReport:
    # Detect file complexity to adjust strategy
    complexity = _detect_file_complexity(path)

    # Adjust strategy based on complexity
    if complexity == "very_complex":
        max_mutants = min(max_mutants, 10)  # Reduce mutants for very complex
        timeout = _PYTEST_TIMEOUT * 2  # Increase timeout
    elif complexity == "complex":
        max_mutants = min(max_mutants, 15)
        timeout = int(_PYTEST_TIMEOUT * 1.5)
    else:
        timeout = _PYTEST_TIMEOUT

    # Find relevant tests for smarter testing
    relevant_tests = _find_relevant_tests(path)
    test_args = list(_PYTEST_ARGS)
    if relevant_tests:
        test_args.extend(relevant_tests)
        print(f"Running targeted tests: {', '.join(relevant_tests)}", file=sys.stderr)
    ...
    # Early termination logic for complex files
    early_check_enabled = (
        complexity != "normal" and len(mutants) > _EARLY_TERMINATION_SAMPLES
    )

    for idx, mutant_source in enumerate(mutants, start=1):
        if early_check_enabled and idx > _EARLY_TERMINATION_SAMPLES:
            # Check if we're consistently killing mutants
            current_score = killed / idx if idx > 0 else 0
            if current_score >= _MUTATION_THRESHOLD * 1.1:  # 10% buffer
                early_terminated = True
                print(
                    f"Early termination: score {current_score:.2%} "
                    f"consistently above threshold",
                    file=sys.stderr,
                )
                break

        complexity_note = f" ({complexity} file)" if complexity != "normal" else ""
        print(f"Testing mutant {idx}/{len(mutants)}{complexity_note}", file=sys.stderr)

        try:
            path.write_text(mutant_source, encoding="utf-8")
            success, output, timed_out = _run_pytest(path, test_args, timeout)
            pytest_output = output
            if timed_out:
                timeout_occurred = True
                print(f"Timeout on mutant {idx}", file=sys.stderr)
        finally:
    ...
    total_tested = idx if early_terminated else len(mutants)
    score = killed / total_tested if total_tested > 0 else 1.0

    return MutationReport(
        score=score,
        total_mutants=total_tested,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if report.passed:
        return 0
    threshold_pct = f"{_MUTATION_THRESHOLD:.0%}"
    print(
        f"Mutation score {report.score:.2%} below threshold {threshold_pct}",
        file=sys.stderr,
    )
    return 1
