...
if ($JsonInput) {
    try {
        $parsed = $JsonInput | ConvertFrom-Json -ErrorAction Stop
        if ($parsed.description) { $featureDesc = $parsed.description }
        else { Write-Error "Json input provided but 'description' key missing"; exit 1 }
        # Log mode if present but enforce canonical
        if ($parsed.mode) { Write-Output "NOTICE: mode '$($parsed.mode)' provided — canonical mode enforced" }
    } catch {
        Write-Error "Failed to parse JsonInput: $_"; exit 1
    }
} else {
    if (-not $FeatureDescription -or $FeatureDescription.Count -eq 0) {
        Write-Error "Usage: ./create-new-feature.ps1 [-Json] [-JsonInput '<json>'] <feature description>"; exit 1
    }
...
Write-Output "Generator mode: canonical"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAg4R7ly9vbMWYt
# XT5dPo6mGuxAfeBdJ8BkgvRQuBxt0qCCFRswggHdMIIBRqADAgECAhB1LVe8LkJa
...
# MQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgWzq0PblbvevgkbKugxjn
# L4J8tB0xRGMZvrOE/qazK4AwDQYJKoZIhvcNAQEBBQAEgYAOfrDxRnBbz3KXoP+6
# 8fUTF47FPihJYtvVU12qDm4r9ikin8IqZ0XCEPi5NCMGCtMIUW/wRJhqbAg6j9Yb
# KiY1cXW7iNBbCuL7vCz1tFur5RiskC/ea025OTWp0XcA3GDIhQWJLpEmjm7gJRSV
# zoTkN/UIZLyhMWbWXHrYiFsML6GCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MjkxNDQ5MTlaMC8GCSqGSIb3DQEJBDEiBCBFlnwykaABJwhSeSHdxNMgNYVV6Omr
# wDOvBKeDwz40NzANBgkqhkiG9w0BAQEFAASCAgDKSuYNPAcfyb4L/Skgww37P8a1
# YIhl0p/4Q0dhqoNhO8Iaacz8ZhnKmHJVk7Z0vzG6I5gLQEDHCJVFYZGy6Cgw11Po
# 6Kw4Y3CFNkoqXhcrNtnAeZzffOA1b8KciPPJ3eR5FQhFBEHkeRBQuxcTYzU6lyFx
# 2JURUCfB/qglyYhqahGm2tQxREUvzapgRyI41891yjkPsbhZiE9MvGWXdJod0QuQ
# ZCpC/PwReENIsehYNiqJKX81U22VwOGL+2m/XpC3Lgzbp1LyCDDk8ajJ+jZXmt2B
# iQhfGXBvpJOhyVbbAUpKHIa7s5pAMmvPjKQTakt8PHKH7aSbdpSP7iZ3JpB5Y0vk
# LBD8BjZopRpLMC9qzselUnoeL3tyJvE4oX2h/SdOWP/u6gjAu7f30c8s2tQevoVQ
# boljDUWeF9dlmGR4hV1knS2E43OEWhsV1p9rXCbVAND1I+n5glM84pqRI5FQSg4w
# h9zDMIt320CrE+o4o/Rf5O/djuAdk52BFs/m5QF05lx4NEpn95zOrDEHGHsNZ3dl
# civsNvNZHNs1WSWI8a4zYUbQ9wDqAOxQRgH/qdicOrg/1lxkHgYTn+s5B5UBCd/T
# DHtM9ueHJJeAz+xcAygxHFbmwW1kzAzN0btReogE8c/82JcxA2xe+bGEjOZcufug
# jhv4C3J06MSW/xdvSQ==
