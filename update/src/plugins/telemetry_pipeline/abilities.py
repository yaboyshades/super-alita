...
Output JSON lines."""
    ...
    @tool
    async def execute(self, task: str, items: list[dict[str, Any]], llm_provider) -> list[dict[str, Any]]:
...
class RankAbility(Ability):
    ...
    @tool
    async def execute(self, task: str, items: list[dict[str, Any]], top_n: int = 200) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            confidence = item.get("confidence", 0.8)
            impact = item.get("signals", {}).get("errors", 0) * 0.1 + \
...
and a draft merged summary (≤80 tokens)."""
    ...
    @tool
    async def execute(self, items: list[dict[str, Any]], llm_provider) -> list[dict[str, Any]]:
...
class PruneAbility(Ability):
    ...
    @tool
    async def execute(self, clusters: list[dict[str, Any]], token_budget: int = 2000) -> list[dict[str, Any]]:
