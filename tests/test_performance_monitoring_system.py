...
from src.performance_monitoring.core.performance_monitor import (
    PerformanceMonitor,
)
from src.performance_monitoring.core.telemetry_bridge import TelemetryBridge
from src.performance_monitoring.dashboard.dashboard_interface import (
    DashboardInterface,
)
...
class TestPerformanceMonitor:
    ...
    def test_extension_statistics(self, performance_monitor):
        ...
        interaction1 = performance_monitor.track_extension_interaction(
            "ext1", "func1"
        )
        performance_monitor.complete_interaction(interaction1, success=True)

        interaction2 = performance_monitor.track_extension_interaction(
            "ext1", "func2"
        )
        performance_monitor.complete_interaction(
            interaction2, success=False, error_message="Test error"
        )
...
class TestTelemetryBridge:
    ...
    def telemetry_bridge(self):
        ...
        return TelemetryBridge(buffer_size=100, flush_interval_seconds=1)
...
class TestConstitutionalEngine:
    ...
    async def test_validate_commit(self, constitutional_engine):
        ...
        result = await constitutional_engine.validate_commit(
            "feat: add new feature", ["src/feature.py"], "diff content"
        )
...
class TestDashboardInterface:
    ...
    def test_constitutional_dashboard(self, dashboard):
        ...
        dashboard._update_metric(
            "constitutional_score", 0.8, "score", "success"
        )
...
class TestQualityGates:
    ...
    async def test_constitutional_gate_validation(self, constitutional_gate):
        ...
        constitutional_gate.constitutional_engine.validate_compliance.return_value = (
            mock_score
        )
    ...
    @pytest.mark.asyncio
    async def test_quality_pipeline_execution(
        self, quality_pipeline, constitutional_gate
    ):
...
class TestEndToEndScenarios:
    ...
    async def test_performance_monitoring_workflow(self):
        ...
        try:
            ...
            interaction = (
                system.performance_monitor.track_extension_interaction(
                    "test_extension", "test_function"
                )
            )
            ...
            system.performance_monitor.complete_interaction(
