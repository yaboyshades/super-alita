...
class TestGitHubApiClient:
    ...
    async def test_priority_metrics_extraction(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            metrics = await self.client.extract_priority_metrics("owner/repo", "pull_request", 456)
...
class TestGitHubPriorityCalculator:
    ...
    def test_priority_explanation(self):
        ...
        assert any("Security alert" in factor for factor in explanation["contributing_factors"])
        assert any("Blocks other PRs" in factor for factor in explanation["contributing_factors"])
...
class TestGitHubIntegrationE2E:
    ...
    def test_priority_calculation_with_github_data(self, sample_priority_metrics):
        ...
        explanation = calculator.get_priority_explanation(sample_priority_metrics, priority)
        
        assert priority > 0.0
        assert explanation["priority_category"] in ["MINIMAL", "LOW", "MEDIUM", "HIGH", "CRITICAL"]
