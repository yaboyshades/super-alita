...
class TestIntegrationReliability:
    ...
    @pytest.mark.asyncio
    async def test_resilient_router_timeout_handling(
        self, mock_base_router, mock_ollama_client
    ):
        ...
        try:
            ...
            await router.executor.execute_with_retry(slow_operation)
    ...
    async def test_circuit_breaker_functionality(self, mock_base_registry):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, recovery_timeout_s=1
        )
    ...
    @pytest.mark.asyncio
    async def test_startup_sequence_mocked(
        self, mock_base_registry, mock_ollama_client
    ):
    ...
    async def test_retry_mechanism_with_eventual_success(
        ...
        retry_config = RetryConfig(
            max_retries=3, base_delay_ms=10, max_delay_ms=50
        )
    ...
    def test_environment_configuration_loading(self):
        ...
        from src.reug_runtime.unified_router import (
            load_resilience_config_from_env,
        )
...
class TestOfflineIntegration:
    ...
    def test_demo_status_offline(self):
        ...
        for _demo_id, demo_info in status.items():
