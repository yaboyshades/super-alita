...
async def test_cortex_adapter_learning_flow():
    ...
    learned = [
        p
        for p in payloads
        if getattr(p, "event_type", getattr(p, "type", None))
...
async def test_gap_event_triggers_reasoning_request():
    ...
    await adapter.handle_knowledge_gap(
        {"data": {"gap_description": "missing rate limiter concept"}}
    )
    assert bus.publish.called
    payload = bus.publish.call_args.args[0]
    assert (
        getattr(payload, "event_type", getattr(payload, "type", None))
        == "reasoning_request"
    )
