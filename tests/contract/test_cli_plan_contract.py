...
def test_cli_plan_output_format() -> None:
    """Contract test for CLI plan command output format."""
    # First create a feature spec to plan
    ...
    # Now run the plan command using the feature ID
    cmd = [
        "python",
        "-m",
        "src.sdd.sdd_cli",
    ...
    plan_length = len(output["artifact_content"])
    assert plan_length > 0, "Implementation plan should not be empty"
