...
class TestEOSLadderPlanner:
    ...
    async def test_eos_ladder_planner_initialization(self):
        ...
        planner1 = EOSLadderPlanner(
            enable_constitutional_analysis=True, enable_mangle_validation=True
        )
    ...
    async def test_simple_goal_planning(self):
        ...
        assert all(
            stage in metadata["stages_completed"] for stage in expected_stages
        )

        print("✓ Simple goal planning completed")
        print(f"  - Stages completed: {metadata['stages_completed']}")
        print(
            f"  - Total tasks: {metadata.get('total_tasks_created', 0) + 1}"
        )  # +1 for root task
        return True
    ...
    async def test_complex_goal_decomposition(self):
        ...
        print(
            f"  - Mangle optimizations: {metadata.get('mangle_optimizations', 0)}"
        )
        print(
            f"  - Constitutional issues: {metadata.get('constitutional_issues', 0)}"
        )
        return True
    ...
    async def test_constitutional_compliance_detection(self):
        ...
        task_graph, metadata = await planner.create_eos_plan(
            goal=self.get_problematic_goal(), context=self.get_basic_context()
        )
        ...
        print(
            f"  - Issues detected: {metadata.get('constitutional_issues', 0)}"
        )
        return True
    ...
    async def test_task_adaptation_mechanisms(self):
        ...
        task_graph, _ = await planner.create_eos_plan(
            goal=self.get_complex_goal(), context=self.get_basic_context()
        )
    ...
    async def test_performance_and_statistics(self):
        ...
        for goal in goals:
            await planner.create_eos_plan(
                goal=goal, context=self.get_basic_context()
            )
        ...
        print(
            f"  - Constitutional compliance rate: {stats.get('constitutional_compliance_rate', 1.0):.2f}"
        )
        return True
    ...
    def test_factory_function(self):
        ...
        planner = create_eos_ladder_planner(
            enable_constitutional_analysis=True, enable_mangle_validation=False
        )
...
async def run_all_tests():
    ...
    try:
        ...
        test_results.append(
            await test_instance.test_eos_ladder_planner_initialization()
        )
        test_results.append(await test_instance.test_mangle_task_validator())
        test_results.append(
            await test_instance.test_constitutional_task_analyzer()
        )
        test_results.append(await test_instance.test_simple_goal_planning())
        test_results.append(
            await test_instance.test_complex_goal_decomposition()
        )
        test_results.append(
            await test_instance.test_constitutional_compliance_detection()
        )
        test_results.append(
            await test_instance.test_task_adaptation_mechanisms()
        )
        test_results.append(await test_instance.test_eos_stage_progression())
        test_results.append(
            await test_instance.test_performance_and_statistics()
        )
        test_results.append(
