...
class TestEnhancedNeuralAtoms:
    ...
    async def test_eos_ladder_processing(self, sample_atom_data):
        ...
        print(
            f"  - Concepts: {len(processed_atom.inferred_properties.get('concepts', []))}"
        )
    ...
    async def test_atom_decomposition(self, complex_atom_data):
        ...
        decomposition_points = processed_atom.inferred_properties[
        ...
        print(
            f"  - Decomposition suggested: {processed_atom.meta.get('decomposition_suggested')}"
        )
    ...
    async def test_constitutional_validation(self):
        ...
        constitutional_validations = [
            v
            for v in processed_atom.validation_results
    ...
    async def test_batch_processing(self, sample_atom_data):
        ...
        for i in range(3):
            ...
            data["content"] = (
                f"This is knowledge item {i+1} about machine learning concepts."
            )
    ...
    async def test_error_handling_and_graceful_degradation(self):
        ...
        try:
            ...
            orchestrator = EOSAtomOrchestrator()
            processed_atom = await orchestrator.process_atom_through_ladder(
                invalid_atom
            )
            ...
            print(
                f"  - Validation issues detected: {len(basic_validations[0].issues)}"
            )
